...
obs_angle = 31
vband_width = 100  # nm
inventory_name = "discrete_inventory.txt"   # filename of the inventory


# Loading np arrays files
files = { f: np.load(f'sources/np_arrays/np_{f}.npy') for f in
                        ['domain', 'coord', 'intensity', 'tech'] }

# Read values from zones inventory
df_zones = pd.read_csv('zones_inventory.csv')
...
sptc = np.loadtxt("Lights/scotopic.dat", skiprows=1)
...
spcts = {spct.split("_")[0] : pt.load_spct(wav, norm_spectrum, f'Datas/spct/{spct}') for spct in list_spcts}
lops = {lop: pt.load_lop(angles, f'Lights/{lop}_pcUPLIGHT.lop') for lop in ['1','5','15']}
